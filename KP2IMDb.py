...
def import_to_imdb_with_methods(fn):
    ...
    def search_and_rating_with_methods(self):
        '''
        ...
        if "No Results" in self.browser.page_source:
            ...
            if int(title_year[1][-5:-1]) != int(self.year):
                return False
...
class IMDb2KP():
    ...
    def _kinopoisk(self):
        ...
        try:
            ...
            self._writelogs('Total films from Kinopoisk: ' + str(len(films_table)))
            ...
            self._writelogs('Total films for IMDb import: ' + str(self.FILMS_qty))
            return True
    ...
    def _importratings(self):
        #for film in self.FILMS:
        for i in range(5):
            film = self.FILMS[i]
            # отдаём query для q
            print(film)
            self.querys = (film['title'] + ' ' + film['year'], film['title'], translatetitle(film['title']))
            ...
            while True:
                try:
                    self.query = self.querys[self.i]
                    if self.import_to_imdb() == True:
                        break
                    self.i += 1
                except IndexError:
                    break
